...
public class NotificationAttachment extends ZBaseEntity {
    ...
    public NotificationAttachment(ODataEntity entity) {
        create(entity);
    }
    ...
    public static ResponseObject getAttachments(String objectKey) {
        ...
        try {
            if (objectKey != null && objectKey.length() > 0) {
                strEntitySet = ZCollections.NO_ATTACHMENT_COLLECTION;
    ...
    private static ResponseObject FromEntity(List<ODataEntity> entities) {
        ...
        try {
            if (entities != null) {
                ArrayList<NotificationAttachment> attachments = new ArrayList<>();
                for (ODataEntity entity : entities) {
                    attachments.add(new NotificationAttachment(entity));
                }
